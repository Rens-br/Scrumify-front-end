...
class WorkItemOption extends Component {
  ...
  getValue = () => {
    return this.state.value !== undefined && this.state.value !== null
      ? this.state.value
      : this.props.default;
  };
  ...
  getListValue = () => {
    return this.state.value !== undefined && this.state.value !== null
      ? this.props.options.find(x => x.id === this.state.value).title
      : this.props.default;
  };
    ...
    switch (type) {
      ...
      case "NumberSelection":
        return (
          <EditableTitle
            ...
            titleChanged={x => this.changeValue(x)}
      ...
      case "ColorSelection":
        return (
          ...
          >
            <div
              style={{ backgroundColor: this.getValue() }}
  ...
  renderSelector = type => {
    switch (type) {
      case "ListSelection":
        return (
          <div>
            <p key={0} className="optionContent">
              {this.props.default}
            </p>
  ...
  render() {
    return (
      <div className="option" ref={node => (this.node = node)}>
        ...
        <div>{this.renderSelection(this.props.type)}</div>
